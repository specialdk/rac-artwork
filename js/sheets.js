...
class SheetsAPI {
  ...
  async fetchSheet(sheetName) {
    ...
    try {
      ...
      throw new Error(
        `Failed to load data from ${sheetName}. Please check your Google Sheet ID and make sure the sheet is public.`
      );
    }
  }
  ...
  async getSiteContent() {
    ...
    const content = {};
    data.forEach((row) => {
      // Use columns A and B since headers aren't being read properly
      if (row.A && row.B && row.A !== "Section") {
        content[row.A] = row.B;
      }
    });
    return content;
